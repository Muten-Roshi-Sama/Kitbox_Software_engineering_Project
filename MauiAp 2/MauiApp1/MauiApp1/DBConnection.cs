...
public class DBConnection{
    ...
    public void addComponent(Component compo){
        Console.WriteLine($"INSERT INTO Components (Reference, Code, LengthC, HeightC, DepthC , Color, PriceSupplier1, DelaySupplier1, PriceSupplier2, DelaySupplier2, StockAvailable,StockOrdered,StockReserved) VALUES('{compo.reference}','{compo.code}',{compo.length},{compo.height},{compo.depth}, {compo.getColorCode()},{compo.priceSupplier1.ToString().Replace(',','.')},{compo.delaySupplier1},{compo.priceSupplier2.ToString().Replace(',','.')},{compo.delaySupplier2},{compo.stockAvailable},{compo.stockOrdered},{compo.stockReserved});");
        using var command = new MySqlCommand($"INSERT INTO Components (Reference, Code, LengthC, HeightC, DepthC , Color, PriceSupplier1, DelaySupplier1, PriceSupplier2, DelaySupplier2, StockAvailable,StockOrdered,StockReserved) VALUES('{compo.reference}','{compo.code}',{compo.length},{compo.height},{compo.depth}, {compo.getColorCode()},{compo.priceSupplier1.ToString().Replace(',','.')},{compo.delaySupplier1},{compo.priceSupplier2.ToString().Replace(',','.')},{compo.delaySupplier2},{compo.stockAvailable},{compo.stockOrdered},{compo.stockReserved});",this.connection);
